...
function cadastro($nome, $login, $email, $senha, $nivel){
    $pdo=conecta();
    try{
        $cadastro = $pdo->prepare("INSERT INTO administrador (administrador_nome, administrador_login, 
                    administrador_email, administrador_senha, administrador_nivel) VALUES(?,?,?,?,?)");
...
function atualizar($nome, $login, $email, $id){
    $pdo=conecta();
    try{
        $atualiza = $pdo->prepare("UPDATE administrador SET administrador_nome = ?, 
                                administrador_login = ?, administrador_email = ? WHERE id = ?");
...
function delete($id){
    ...
    try{
        $delete = $pdo->prepare("DELETE FROM administrador WHERE id = ?");
